...
with DAG(
    ...
    @task()
    def allocate_workers(file_pattern,max_workers=max_workers,max_files_per_worker=max_files_per_worker,max_files_per_asset=max_files_per_asset) -> List[list]:  # [[{asset->filemanes}]]
        """
        #### Extract task
                ...
                for file_disposition in asset_file_dispositions['dispositions']:
                    ...
                    if(file_disposition["processed"] is True and file_disposition["errors"] is False):
                        ...
                        if(not(sftp_exists(sftp,processed_path))):
                            ...
                            print(f"ERROR: file {processed_path} already exists, this must contain duplicate timestamps!!! - moving file to errors directory path")
                            duplicate=True
                    if(duplicate or (file_disposition["processed"] is True and file_disposition["errors"] is True)):
                        ...
                        if(sftp_exists(sftp,errors_path)):
                            print(f"ERROR: file {errors_path} already exists, this must contain duplicate timestamps!!!")
